...
export interface DashboardV2Alert {
    ...
    actionLabel: string;
    actionHref: string;
    ruleId: 'HIGH_NEGATIVE_SPIKE' | 'STALE_SYNC' | 'LOW_ANALYSIS_COVERAGE' | 'PENDING_WORKLOAD';
}
...
export type ClientSummaryAlertRuleId = 'NO_CONNECTED_ACCOUNT' | 'STALE_SYNC' | 'LOW_ANALYSIS_COVERAGE' | 'HIGH_NEGATIVE_RATE' | 'PENDING_WORKLOAD';
export interface ClientSummaryV2Response {
    clientHeader: Pick<ClientContract, 'id' | 'name' | 'shortName' | 'industry' | 'connected' | 'status' | 'avatarColor'>;
    statusOverview: {
